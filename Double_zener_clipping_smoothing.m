...
for i = 1:length(time)
    ...
    % Step 2: Determine the state of diode 1(positive Zener diode) based on previous temperature difference
     if delta_T_diode1_prev > 0
        % Forward
        R_diode1 = R_forward;
    elseif delta_T_diode1_prev <= - (T_breakdown + buffer_range)
        % Reverse breakdown
    ...
    elseif delta_T_diode1_prev >= - (T_breakdown - buffer_range)
        ...
        R_diode1 = R_reverse + (R_breakdown - R_reverse) * ((-delta_T_diode1_prev - (T_breakdown - buffer_range)) / (2 * buffer_range));
     end
    
    % Step 3: Determine the state of diode 2(negative Zener Diode) based on previous temperature difference
    if delta_T_diode2_prev < 0
        % Forward
        R_diode2 = R_forward;
    elseif delta_T_diode2_prev >= (T_breakdown + buffer_range)
        % Reverse breakdown
    ...
    elseif delta_T_diode2_prev <= (T_breakdown - buffer_range)
        ...
        % Linear transition between reverse blocking and breakdown
        R_diode2 = R_reverse + (R_breakdown - R_reverse) * ((delta_T_diode2_prev - (T_breakdown - buffer_range)) / (2 * buffer_range));
    end
